...
static bool snprintk_putchar(char c, snprintk_state_t* state)
{
...
static bool snprintk_print(const char* data, size_t length, snprintk_state_t* state)
{
	for (size_t i = 0; i < length; i++) {
		if (!snprintk_putchar(data[i], state))
			return false;
	}
	return true;
}
...
int snprintk(char* restrict buffer, size_t size, const char* restrict format, ...)
{
	if (size == 0)
		return -1;
	...
	while (*format != '\0') {
		...
		if (format[0] != '%' || format[1] == '%') {
			...
			if (!snprintk_print(format, amount, &state))
				goto overflow;
		...
		if (*format == 'c') {
			...
			if (!maxrem || !snprintk_putchar(c, &state))
				goto overflow;
			written++;
			...
			if (maxrem < len || !snprintk_print(str, len, &state))
				goto overflow;
			written += len;
			...
			if (maxrem < len || !snprintk_print(buf, len, &state))
				goto overflow;
			written += len;
			...
			if (*format == 'l') {
				...
				if (*format != 'u') {
					...
					if (maxrem < len || !snprintk_print(format, len, &state))
						goto overflow;
					written += len;
					format += len;
					continue;
			...
			if (maxrem < (len1 + len2))
				goto overflow;
			if (!snprintk_print(prefix, 2, &state) || !snprintk_print(buf, len2, &state))
				goto overflow;
			written += len1 + len2;
