...
// User mode stack for the built-in user_main - allocated in the dedicated
// user section so ring 3 can use it.
static uint32_t user_stack[1024] __attribute__((aligned(16), section(".user")));

extern void user_main(void);

// Construct an IRET frame to switch to Ring 3 at (entry_point, stack_top).
// The IRET instruction pops: EIP, CS, EFLAGS, [ESP, SS if privilege level
// changed]. We push in reverse order: SS, ESP, EFLAGS, CS, EIP.
...
void enter_user_mode(void)
{
    ...
    enter_user_mode_at((uint32_t)&user_main, user_stack_top);
}
